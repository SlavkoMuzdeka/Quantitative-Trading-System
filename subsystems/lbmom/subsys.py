...
class Lbmom:
    ...
    def run_simulation(self, historical_data):
        ...
        for i in portfolio_df.index:
            ...
            """
            if i != 0:
                data_prev = portfolio_df.loc[i - 1, "date"]
            ...
            for inst in tradable:
                ...
                for pair in self.pairs:
                    ...
                    forecast = (
                        0
                        if historical_data.loc[date, "{} adx".format(inst)]
                        else forecast
                    )  # check if regime is trending else cast 0 vote for all
                    ...
                    position = strat_scalar * forecast * vol_target / dollar_volatility
            ...
            for inst in tradable:
                units = portfolio_df.loc[i, "{} units".format(inst)] = position
                nominal_inst = abs(
                    position * historical_data.loc[date, "{} close".format(inst)]
                )
